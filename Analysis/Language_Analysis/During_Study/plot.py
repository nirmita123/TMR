...
base2='../../../Plots/Language_Plots/During_Study/'
...
def plotSignificance(score_control,score_scent,ax):
    ...
    Pval= Pval/2
    y, h, col = max((score_control.mean())*100+stats.sem(score_control)*100,(score_scent.mean())*100+stats.sem(score_scent)*100), 3, 'k'
    if(Pval<0.0001):
        star="***"
    elif Pval<0.001:
        star="**"
...
#     print("Control",st.mean(change_control),"+/-",np.std(change_control),"Scent",st.mean(change_scent),"+/-",np.std(change_scent))
    ...
    P=plotSignificance(np.asarray(data1)/100,np.asarray(data2)/100,ax)
...
def plot_with_broken_axis(title, figure_name, xlabel1, xlabel2, color1, color2, ylabel, data1, data2, ylim, ytext):
    ax,ax2=broken_axis(ylim)
    barWidth=0.25

    # calculate mean and standard error mean
    mean_control_object= st.mean(data1)*100
    mean_scent_object= st.mean(data2)*100
    sem_control_object= stats.sem(data1)*100
    sem_scent_object= stats.sem(data2)*100
    ...
    ttl = ax.title
    ttl.set_position([.5, 1.05])
    ax.text(-0.2, ylim[1]+1, "%", ha='left',fontsize = 14)
    ...
    ax.text(.02, ylim[0]-ytext, txt1, ha='left')
    plt.savefig(base2+figure_name,dpi=300,bbox_inches='tight')
